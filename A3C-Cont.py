...
class ActorCritic(nn.Module):
    ...
    def calc_R(self, done):
        states = torch.tensor(self.next_states, dtype=torch.float32)
    ...
    def run(self):
        ...
        while self.episode_idx.value < self.num_iter:
            ...
            while not done:
                ...
                self.local_actor_critic.remember(np.array([curr_state,time]), action, reward, done, np.array([next_state,time+1]))
            ...
            with self.episode_idx.get_lock():
                ...
                if(self.episode_idx.value%10)==0:
                    find_threshold(self.global_actor_critic,drift, start_price, T, episode=self.episode_idx.value)
            print(self.name, 'episode ', self.episode_idx.value, 'reward %.1f' % reward, 'loss ', loss.item())


def find_threshold(model, drift, start_price, T, episode):
    with torch.no_grad():
        ...
        # Write policy for each time-price pair to the file
            for time, price_policy in enumerate(thresholds):
...
if __name__ == '__main__':
    ...
    optimal_policy=find_threshold(global_actor_critic, drift, start_price, T, 0)
